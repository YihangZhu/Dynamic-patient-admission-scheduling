...
const string instance_xml_schema = "Instances/newGen/OrPasInstance.xsd";
const string output_xml_schema = "Instances/newGen/OrPasSolution.xsd";
...
{ 
  ...
  unsigned idle_room_cost1 = 0;
  unsigned idle_operating_room_cost1 = 0;
  ...
  // room capacity cost
//    unsigned totalUsedCapacity1 = 0, totalRoomCapacity1 = 0;
  for (r = 0; r < num_rooms; r++)
    for (d = 0; d < num_days; d++)
      {
	for (p = 0; p < num_patients; p++)
  ...
  for (p = 0; p < num_patients; p++)
    ...
    unsigned ORUsedCapacity = 0, ORCapacity = 0, totalAdmittedOvertime = 0;;
    //unsigned ORO = 0, ORTO = 0;
      for (d = 0; d < num_days; d++)
    {
      for (spec = 0; spec < Specialisms(); spec++)
	{
	  ...
	  // Operating Room Overtime
        ORUsedCapacity += or_occupancy[d][spec];
        ORCapacity += OrSlots(d, spec);
	  if(or_occupancy[d][spec] > OrSlots(d, spec))
	    {
            totalAdmittedOvertime += NumOrSlots(d, spec);
	      if(or_occupancy[d][spec] > OrSlots(d, spec) + AdmittedOvertime(d, spec)) 
...
//    cout<<"totalAdmittedOvertime:"<<totalAdmittedOvertime<<endl;
     //cout<<"ORCapacity:" << ORCapacity<<endl;
  // gender cost
	...
	if (RoomGenderPolicy(r) == SAME_GENDER)
	  {
	    ...
	    {
            if(rgConstr == "simplified"){
               gender_cost += 1;
            }else if(rgConstr == "minorValue"){
               gender_cost += min(room_males[r], room_females[r]);
            }
	    }
	  }
    }
  ...
  // overcrowd risk cost
//    int total_occupancy = 0, total_room_capacity=0;
  for (r = 0; r < num_rooms; r++)
    for (d = 1; d < num_days; d++)
      {

	potential_occupancy = occupancy[r][d];
    //total_occupancy += occupancy[r][d];
    //total_room_capacity += GetRoom(r)->capacity;
	for (p = 0; p < num_patients; p++)
  ...
  //cout<<"total_occupancy"<<total_occupancy<<endl;
  //cout<<"total_roomCapacity"<<total_room_capacity<<endl;
  //IdleOperatingRoom
    idle_operating_room_cost = 0 ;//
    idle_operating_room_cost1 = min(total_or_requests, total_or_slots);
    int temp = total_or_slots - total_or_requests;
    int diff = max(temp, 0); // get the redundant capacity during the original horizon

    for (d = 0; d < OriginalPlanningHorizon(); d++)
        for (spec = 0; spec < num_specialisms; spec++) {
            if (or_occupancy[d][spec] < OrSlots(d, spec)) {
                idle_operating_room_cost +=  OrSlots(d,spec) - or_occupancy[d][spec];
                idle_operating_room_cost1 -=  or_occupancy[d][spec];
            } else {
                idle_operating_room_cost1 -= OrSlots(d, spec);
            }
        }
    idle_operating_room_cost -= diff;
    idle_operating_room_cost *= IDLE_OPERATING_ROOM_WEIGHT;
    idle_operating_room_cost1 *= IDLE_OPERATING_ROOM_WEIGHT;

    if(idle_operating_room_cost != idle_operating_room_cost1){
        cout<<"idle operating room costs are different!"<<endl;
        cout<<"idle: "<<idle_operating_room_cost<<"\tidle1: "<<idle_operating_room_cost1<<endl;

    }

  // IdleRoomCapacity
  idle_room_cost = 0 ;
  idle_room_cost1 = min(total_room_requests, num_beds * OriginalPlanningHorizon());
  temp = num_beds * OriginalPlanningHorizon()-total_room_requests;
  diff = max(0,temp);
  for (d = 0; d < OriginalPlanningHorizon(); d++)
    {
      for (r = 0; r < num_rooms; r++) {
          if (occupancy[r][d] < GetRoom(r)->capacity) {
              idle_room_cost += GetRoom(r)->capacity - occupancy[r][d];
              idle_room_cost1 -= occupancy[r][d];
          }else{
            idle_room_cost1 -= GetRoom(r)->capacity;
          }
      }
    }
    idle_room_cost -= diff;
 idle_room_cost *= IDLE_ROOM_CAPACITY_WEIGHT;
 idle_room_cost1 *= IDLE_ROOM_CAPACITY_WEIGHT;
 if(idle_room_cost != idle_room_cost1){
         cout<<"idle room costs are different!"<<endl;
         cout<<"capacity: "<<min(total_room_requests, num_beds * OriginalPlanningHorizon())<<endl;
         cout<<"diff: "<<diff<<endl;
         cout<<"idle: "<<idle_room_cost<<"\tidle1: "<<idle_room_cost1<<endl;
     }
...
int main(int argc, char* argv[]) 
{
  if (argc!= 3 && argc !=4)
    {
      cerr << "Usage: " << argv[0] << " <input file>* <solution file>* <gender policy violation cost: simplified or minorValue>" << endl;
      return 1;
    }
  ...
  if(argc == 4){
    ...
    rgConstr = "simplified"; // "minorValue"
  }
